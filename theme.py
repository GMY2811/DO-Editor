...
/* 格式编辑模块(FormatDialog)的加粗/斜体开关：14pt 大字面。
   全局 QPushButton 的 padding:6px 16px 会上下各啃 6px, 把 B/I 字母
   下半截裁没。这里 ID 规则用 padding:0 覆盖全局 padding, 让 14pt
   字符完整显示; 不动 min-width/min-height, 让 sizeHint+fixedSize
   一起把按钮撑到合理高度(34), 同行四控件对齐。 */
...
/* 深色主题：格式编辑模块 B/I 开关（padding 归零防裁 14pt 字符） */
QPushButton#textFormatToggle {
    background: #3a3a3c;
    color: #f2f2f7;
...
def _resolve_dark(mode, dark_override=None):
    if mode == "dark":
        return True
    if mode == "system":
        if dark_override is not None:
            return bool(dark_override)
...
def is_dark(mode, dark_override=None):
    return _resolve_dark(mode, dark_override)
